...
export async function executeTrackInfo(interaction: any): Promise<void> {
  ...
  let trackName: string | null = rawTrack?.includes("|||")
    ? rawTrack.split("|||")[0]!
    : rawTrack;
  let artistName: string | null = rawTrack?.includes("|||")
    ? rawTrack.split("|||")[1]!
    : null;
  ...
  if (!trackName || !artistName) {
    if (!lfmUsername) {
      const container = new ContainerBuilder().addTextDisplayComponents(
        new TextDisplayBuilder().setContent(
          `${E.reject} No track specified and you haven't linked your Last.fm. Use ${cmdMention("link")} or specify a track.`,
        ),
      );
    ...
    if (!np?.trackName || !np?.artistName) {
      const container = new ContainerBuilder().addTextDisplayComponents(
        new TextDisplayBuilder().setContent(
          `${E.reject} Couldn't detect what you're listening to. Please specify a track.`,
        ),
      );
      await interaction.editReply({
        components: [container],
  ...
  const [lfmData, personalData] = await Promise.all([
    fetch(
      `https://ws.audioscrobbler.com/2.0/?method=track.getInfo&track=${encodeURIComponent(trackName)}&artist=${encodeURIComponent(artistName)}&api_key=${apiKey}&format=json`,
    )
      .then((r) => r.json())
      .catch(() => null),
    lfmUsername
      ? fetch(
          `https://ws.audioscrobbler.com/2.0/?method=track.getInfo&track=${encodeURIComponent(trackName)}&artist=${encodeURIComponent(artistName)}&username=${encodeURIComponent(lfmUsername)}&api_key=${apiKey}&format=json`,
        )
          .then((r) => r.json())
          .catch(() => null)
  ...
  if (!lfmData || lfmData.error) {
    const container = new ContainerBuilder().addTextDisplayComponents(
      new TextDisplayBuilder().setContent(
        `${E.reject} Couldn't find **${trackName}** by **${artistName}** on Last.fm.`,
      ),
    );
    await interaction.editReply({
      components: [container],
  ...
  const cleanWiki = rawWiki
    ? rawWiki.replace(/<a\s[^>]*>.*?<\/a>/gi, "").trim()
    : "";
  let wiki: string | null = null;
  if (cleanWiki) {
    const sentences = cleanWiki.match(/[^.!?]+[.!?]+/g) ?? [];
    const twoSentences =
      sentences.slice(0, 2).join(" ").trim() || cleanWiki.slice(0, 200);
  ...
  const lfmAlbumImage =
    track?.album?.image?.find((img: any) => img.size === "extralarge")?.[
      "#text"
    ] || null;
  const lfmImageValid =
    lfmAlbumImage && !lfmAlbumImage.includes(LFM_PLACEHOLDER)
      ? lfmAlbumImage
      : null;

  let imageUrl: string | null = lfmImageValid;
  if (!imageUrl) {
    const itunesQuery = albumName
    ...
    const match =
      results.find((r: any) =>
        r[nameField]?.toLowerCase().includes(nameLower),
      ) ??
      results[0] ??
      null;
  ...
  if (personalPlaycount) {
    ...
    container.addTextDisplayComponents(
      new TextDisplayBuilder().setContent(playsLine),
    );
  }
