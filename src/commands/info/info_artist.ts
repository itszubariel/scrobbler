...
export async function executeArtistInfo(interaction: any): Promise<void> {
  ...
  const [lfmData, deezerData, personalData] = await Promise.all([
    ...
    lfmUsername
      ? fetch(
          `https://ws.audioscrobbler.com/2.0/?method=artist.getInfo&artist=${encodeURIComponent(artistName)}&username=${encodeURIComponent(lfmUsername)}&api_key=${apiKey}&format=json`,
        )
          .then((r) => r.json())
          .catch(() => null)
  ...
  if (!lfmData || lfmData.error) {
    const container = new ContainerBuilder().addTextDisplayComponents(
      new TextDisplayBuilder().setContent(
        `${E.reject} Couldn't find artist **${artistName}** on Last.fm.`,
      ),
    );
    await interaction.editReply({
      components: [container],
  ...
  const deezerMatch =
    deezerResults.find(
      (r: any) => r.name.toLowerCase() === artistName!.toLowerCase(),
    ) ??
    deezerResults[0] ??
    null;
  ...
  const playcount = parseInt(artist?.stats?.playcount ?? "0").toLocaleString(
    "en-US",
  );
  const tags: string[] = (
    ...
    .slice(0, 3)
    .map((t: any) => t.name);
  const similar: string[] = (artist?.similar?.artist ?? [])
    .slice(0, 5)
    .map((a: any) => a.name);
  ...
  if (cleanBio) {
    ...
    const twoSentences =
      sentences.slice(0, 2).join(" ").trim() || cleanBio.slice(0, 300);
  ...
  const similarLine =
    similar.length > 0 ? `**Similar:** ${similar.join(", ")}` : null;
  const personalLine = personalPlaycount
  ...
  if (personalLine) {
    container.addTextDisplayComponents(
      new TextDisplayBuilder().setContent(personalLine),
    );
  }

  if (similarLine) {
    container.addTextDisplayComponents(
      new TextDisplayBuilder().setContent(similarLine),
    );
  }
