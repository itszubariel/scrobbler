...
export async function executeAlbumInfo(interaction: any): Promise<void> {
  ...
  let albumName: string | null = rawAlbum?.includes("|||")
    ? rawAlbum.split("|||")[0]!
    : rawAlbum;
  let artistName: string | null = rawAlbum?.includes("|||")
    ? rawAlbum.split("|||")[1]!
    : null;
  ...
  if (!albumName || !artistName) {
    ...
    if (!np?.albumName || !np?.artistName) {
      const container = new ContainerBuilder().addTextDisplayComponents(
        new TextDisplayBuilder().setContent(
          `${E.reject} Couldn't detect what you're listening to. Please specify an album.`,
        ),
      );
      await interaction.editReply({
        components: [container],
  ...
  const [lfmData, personalData] = await Promise.all([
    fetch(
      `https://ws.audioscrobbler.com/2.0/?method=album.getInfo&album=${encodeURIComponent(albumName)}&artist=${encodeURIComponent(artistName)}&api_key=${apiKey}&format=json`,
    )
      .then((r) => r.json())
      .catch(() => null),
    lfmUsername
      ? fetch(
          `https://ws.audioscrobbler.com/2.0/?method=album.getInfo&album=${encodeURIComponent(albumName)}&artist=${encodeURIComponent(artistName)}&username=${encodeURIComponent(lfmUsername)}&api_key=${apiKey}&format=json`,
        )
          .then((r) => r.json())
          .catch(() => null)
  ...
  if (!lfmData || lfmData.error) {
    const container = new ContainerBuilder().addTextDisplayComponents(
      new TextDisplayBuilder().setContent(
        `${E.reject} Couldn't find **${albumName}** by **${artistName}** on Last.fm.`,
      ),
    );
    await interaction.editReply({
      components: [container],
  ...
  const lfmImageValid =
    lfmImage && !lfmImage.includes(LFM_PLACEHOLDER) ? lfmImage : null;
  ...
  const totalDurationSecs = tracksRaw.reduce(
    (sum: number, t: any) => sum + (parseInt(t.duration ?? "0") || 0),
    0,
  );
  const totalDurationStr =
    totalDurationSecs > 0 ? formatDuration(totalDurationSecs) : null;
  ...
  if (trackListStr) {
    container
      ...
      .addTextDisplayComponents(
        new TextDisplayBuilder().setContent(trackListStr),
      );
  }
