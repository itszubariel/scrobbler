...
export async function executeGenreInfo(interaction: any): Promise<void> {
  ...
  const [tagData, topArtistsData] = await Promise.all([
    fetch(
      `https://ws.audioscrobbler.com/2.0/?method=tag.getInfo&tag=${encodeURIComponent(genreName)}&api_key=${apiKey}&format=json`,
    )
      .then((r) => r.json())
      .catch(() => null),
    fetch(
      `https://ws.audioscrobbler.com/2.0/?method=tag.gettopartists&tag=${encodeURIComponent(genreName)}&limit=10&api_key=${apiKey}&format=json`,
    )
      .then((r) => r.json())
      .catch(() => null),
  ]);
  ...
  if (!tagData || tagData.error) {
    const container = new ContainerBuilder().addTextDisplayComponents(
      new TextDisplayBuilder().setContent(
        `${E.reject} Couldn't find genre **${genreName}** on Last.fm.`,
      ),
    );
    await interaction.editReply({
      components: [container],
  ...
  if (cleanWiki) {
    ...
    const twoSentences =
      sentences.slice(0, 2).join(" ").trim() || cleanWiki.slice(0, 300);
