...
export async function executeTopAlbums(interaction: any): Promise<void> {
  ...
  // Check cache first
  const cacheKey = `chart_albums_${targetDiscordUser.id}_${period}_${size}`;
    ...
    const container = new ContainerBuilder()
      .addTextDisplayComponents(
        new TextDisplayBuilder().setContent(
          `### ${E.albums} ${lfmUsername}'s Top Albums — ${periodLabel}`,
        ),
      )
      ...
      .addTextDisplayComponents(
        new TextDisplayBuilder().setContent(
          `-# Top ${count} albums • ${size} • ${periodLabel}`,
        ),
      );
  ...
  if (albums.length === 0) {
    const container = new ContainerBuilder().addTextDisplayComponents(
      new TextDisplayBuilder().setContent(
        `${E.reject} No top albums found for **${lfmUsername}** in this period.`,
      ),
    );
    await interaction.editReply({
      components: [container],
  ...
  const imageResults = await Promise.all(
    albums.map((a) => {
      ...
      if (lfmImage && !lfmImage.includes(LFM_PLACEHOLDER))
        return Promise.resolve({ _lfm: lfmImage });
      return fetch(
        `https://itunes.apple.com/search?term=${encodeURIComponent((a.artist?.name ?? "") + " " + a.name)}&entity=album&limit=1`,
      )
        .then((r) => r.json())
        .catch(() => null);
    }),
  ...
  const imageUrl = await uploadToSupabase(
    buffer,
    "chart-cache",
    `${targetDiscordUser.id}_albums_${period}_${size}.png`,
  );
  ...
  const container = new ContainerBuilder()
    ...
    .addTextDisplayComponents(
      new TextDisplayBuilder().setContent(
        `-# Top ${items.length} albums • ${size} • ${periodLabel}`,
      ),
    );
