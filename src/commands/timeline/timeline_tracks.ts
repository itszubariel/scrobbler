...
// Fetch total scrobble count for a user in a time window (total tracks played)
async function fetchScrobbleCount(
  ...
  // Use page 1 with limit 1 just to get the total attribute
...
export const timelineTracksCommand = {
  execute: async (interaction: any) => {
    ...
    const cacheKey = `timeline_tracks_${targetDiscordUser.id}_${months}`;
    ...
    const counts = await Promise.all(
      windows.map((w) => fetchScrobbleCount(lfmUsername, apiKey, w.from, w.to)),
    );
    ...
    const buffer = await buildTimelineCanvas(
      series,
      `${lfmUsername}'s Track Timeline`,
      `Scrobbles per month — last ${months} months`,
    );

    const imageUrl = await uploadToSupabase(buffer, "timeline-cache", `${targetDiscordUser.id}_tracks_${months}.png`);
...
function buildContainer(imageUrl: string, lfmUsername: string, months: number) {
  return new ContainerBuilder()
    .addTextDisplayComponents(
      new TextDisplayBuilder().setContent(`### ${E.tracks} ${lfmUsername}'s Track Timeline`),
    )
    ...
    .addTextDisplayComponents(
      new TextDisplayBuilder().setContent(`-# Scrobbles per month • Last ${months} months`),
    );
}
