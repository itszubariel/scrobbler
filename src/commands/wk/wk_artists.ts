...
export async function executeWkArtists(interaction: any): Promise<void> {
  ...
  if (!artistName) {
    ...
    if (!callerDb?.lastfmUsername) {
      const container = new ContainerBuilder().addTextDisplayComponents(
        new TextDisplayBuilder().setContent(
          `${E.reject} No artist specified and you haven't linked your Last.fm. Use ${cmdMention("link")} or specify an artist.`,
        ),
      );
    ...
    if (!np) {
      const container = new ContainerBuilder().addTextDisplayComponents(
        new TextDisplayBuilder().setContent(
          `${E.reject} Couldn't detect what you're listening to. Please specify an artist.`,
        ),
      );
      await interaction.editReply({
        components: [container],
  ...
  const results = (await Promise.all(
    linkedMembers.map((m) =>
      fetch(
        `https://ws.audioscrobbler.com/2.0/?method=artist.getInfo&artist=${encodeURIComponent(artistName!)}&username=${encodeURIComponent(m.user.lastfmUsername!)}&api_key=${apiKey}&format=json`,
      )
        .then((r) => r.json())
        .catch(() => null),
    ),
  ...
  if (allSorted.length === 0) {
    const container = new ContainerBuilder().addTextDisplayComponents(
      new TextDisplayBuilder().setContent(
        `${E.reject} Nobody in this server has listened to **${artistName}**.`,
      ),
    );
    await interaction.editReply({
      components: [container],
  ...
  const canonicalName =
    results.find((r) => r?.artist?.name)?.artist?.name ?? artistName;
  ...
  const urls = await Promise.all(
    buffers.map((buf, i) =>
      uploadToSupabase(
        buf,
        "wk-cache",
        `${interaction.guildId}_artist_${encodeURIComponent(canonicalName)}_${i}.png`,
      ),
    ),
  );

  // Save to generic cache
  const genericCacheKey = `wk_artists_${interaction.guildId}_${canonicalName}`;
  await setCache(
    genericCacheKey,
    {
      imageUrls: urls,
  ...
  const container = new ContainerBuilder()
    .addTextDisplayComponents(
      new TextDisplayBuilder().setContent(
        `### ${E.artists} Who Knows **${canonicalName}**?`,
      ),
    )
  ...
  if (totalPages > 1) {
    const row = new ActionRowBuilder().addComponents(
      new ButtonBuilder()
        .setCustomId(
          `wk_artist_prev_0_${interaction.guildId}_${encodeURIComponent(canonicalName)}`,
        )
        .setEmoji({
      ...
      new ButtonBuilder()
        .setCustomId(
          `wk_artist_next_0_${interaction.guildId}_${encodeURIComponent(canonicalName)}`,
        )
        .setEmoji({
