...
export async function executeWkArtists(interaction: any): Promise<void> {
  ...
  if (!artistName) {
    ...
    if (!callerDb?.lastfmUsername) {
      const container = new ContainerBuilder().addTextDisplayComponents(new TextDisplayBuilder().setContent(`${E.reject} No artist specified and you haven't linked your Last.fm. Use ${cmdMention('link')} or specify an artist.`));
      await interaction.editReply({ components: [container], flags: MessageFlags.IsComponentsV2 });
      return;
    }
    const np = await fetchNowPlaying(callerDb.lastfmUsername, apiKey);
    if (!np) {
      const container = new ContainerBuilder().addTextDisplayComponents(new TextDisplayBuilder().setContent(`${E.reject} Couldn't detect what you're listening to. Please specify an artist.`));
      await interaction.editReply({ components: [container], flags: MessageFlags.IsComponentsV2 });
      return;
    }
  ...
  const allSorted = linkedMembers
    .map((m, i) => ({ username: m.user.lastfmUsername!, plays: parseInt(results[i]?.artist?.stats?.userplaycount ?? '0') }))
    .filter(m => m.plays > 0)
    .sort((a, b) => b.plays - a.plays);

  if (allSorted.length === 0) {
    const container = new ContainerBuilder().addTextDisplayComponents(new TextDisplayBuilder().setContent(`${E.reject} Nobody in this server has listened to **${artistName}**.`));
    await interaction.editReply({ components: [container], flags: MessageFlags.IsComponentsV2 });
    return;
  }
  ...
  const cacheKey = `artist:${canonicalName}`;

  const buffers = await Promise.all(
    Array.from({ length: totalPages }, (_, i) => buildWkCanvas(allSorted, `Who Knows — ${canonicalName}`, 'artist', 'plays', interaction.guild.name, i))
  );
  const urls = await Promise.all(
    buffers.map((buf, i) => uploadToSupabase(buf, 'wk-cache', `${interaction.guildId}_artist_${encodeURIComponent(canonicalName)}_${i}.png`))
  );
  ...
  else if (callerLfm && callerRank === 0) footerParts.push(`You haven't listened to this artist`);

  const container = new ContainerBuilder()
    .addTextDisplayComponents(new TextDisplayBuilder().setContent(`### ${E.artists} Who Knows **${canonicalName}**?`))
  ...
  if (totalPages > 1) {
    const row = new ActionRowBuilder().addComponents(
      new ButtonBuilder().setCustomId(`wk_artist_prev_0_${interaction.guildId}_${encodeURIComponent(canonicalName)}`).setEmoji({ id: E.prev.match(/:(\d+)>/)?.[1] ?? '0', name: 'scrobbler_prev' }).setStyle(ButtonStyle.Secondary).setDisabled(true),
      new ButtonBuilder().setCustomId(`wk_artist_next_0_${interaction.guildId}_${encodeURIComponent(canonicalName)}`).setEmoji({ id: E.next.match(/:(\d+)>/)?.[1] ?? '0', name: 'scrobbler_next' }).setStyle(ButtonStyle.Secondary).setDisabled(false),
    );
