...
export async function executeWkAlbums(interaction: any): Promise<void> {
  ...
  if (!albumInput || !artistInput) {
    ...
    if (!callerDb?.lastfmUsername) {
      const container = new ContainerBuilder().addTextDisplayComponents(new TextDisplayBuilder().setContent(`${E.reject} No album specified and you haven't linked your Last.fm. Use ${cmdMention('link')} or specify an album.`));
      await interaction.editReply({ components: [container], flags: MessageFlags.IsComponentsV2 });
      return;
    }
    const np = await fetchNowPlaying(callerDb.lastfmUsername, apiKey);
    if (!np?.albumName) {
      const container = new ContainerBuilder().addTextDisplayComponents(new TextDisplayBuilder().setContent(`${E.reject} Couldn't detect what album you're listening to. Please specify one.`));
      await interaction.editReply({ components: [container], flags: MessageFlags.IsComponentsV2 });
      return;
    }
  ...
  const allSorted = linkedMembers
    .map((m, i) => ({ username: m.user.lastfmUsername!, plays: parseInt(results[i]?.album?.userplaycount ?? '0') }))
    .filter(m => m.plays > 0)
    .sort((a, b) => b.plays - a.plays);

  if (allSorted.length === 0) {
    const container = new ContainerBuilder().addTextDisplayComponents(new TextDisplayBuilder().setContent(`${E.reject} Nobody in this server has listened to **${albumInput}** by **${artistInput}**.`));
    await interaction.editReply({ components: [container], flags: MessageFlags.IsComponentsV2 });
    return;
  }
  ...
  const buffers = await Promise.all(
    Array.from({ length: totalPages }, (_, i) => buildWkCanvas(allSorted, `Who Knows — ${canonicalAlbum}`, `album by ${canonicalArtist}`, 'plays', interaction.guild.name, i))
  );
  const urls = await Promise.all(
    buffers.map((buf, i) => uploadToSupabase(buf, 'wk-cache', `${interaction.guildId}_album_${encodeURIComponent(canonicalAlbum)}_${i}.png`))
  );
  ...
  else if (callerLfm && callerRank === 0) footerParts.push(`You haven't listened to this album`);

  const container = new ContainerBuilder()
    .addTextDisplayComponents(new TextDisplayBuilder().setContent(`### ${E.albums} Who Knows **${canonicalAlbum}**?`))
  ...
  if (totalPages > 1) {
    const row = new ActionRowBuilder().addComponents(
      new ButtonBuilder().setCustomId(`wk_album_prev_0_${interaction.guildId}_${encodeURIComponent(canonicalAlbum)}|||${encodeURIComponent(canonicalArtist)}`).setEmoji({ id: E.prev.match(/:(\d+)>/)?.[1] ?? '0', name: 'scrobbler_prev' }).setStyle(ButtonStyle.Secondary).setDisabled(true),
      new ButtonBuilder().setCustomId(`wk_album_next_0_${interaction.guildId}_${encodeURIComponent(canonicalAlbum)}|||${encodeURIComponent(canonicalArtist)}`).setEmoji({ id: E.next.match(/:(\d+)>/)?.[1] ?? '0', name: 'scrobbler_next' }).setStyle(ButtonStyle.Secondary).setDisabled(false),
    );
