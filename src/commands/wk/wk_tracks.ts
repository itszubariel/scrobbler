...
export async function executeWkTracks(interaction: any): Promise<void> {
  ...
  const results = (await Promise.all(
    linkedMembers.map((m) =>
      fetch(
        `https://ws.audioscrobbler.com/2.0/?method=track.getInfo&artist=${encodeURIComponent(artistInput!)}&track=${encodeURIComponent(trackInput!)}&username=${encodeURIComponent(m.user.lastfmUsername!)}&api_key=${apiKey}&format=json`,
      )
        .then((r) => r.json())
        .catch(() => null),
    ),
  ...
  if (allSorted.length === 0) {
    const container = new ContainerBuilder().addTextDisplayComponents(
      new TextDisplayBuilder().setContent(
        `${E.reject} Nobody in this server has listened to **${trackInput}** by **${artistInput}**.`,
      ),
    );
    await interaction.editReply({
      components: [container],
  ...
  const canonicalTrack =
    results.find((r) => r?.track?.name)?.track?.name ?? trackInput;
  const canonicalArtist =
    results.find((r) => r?.track?.artist?.name)?.track?.artist?.name ??
    artistInput;
  ...
  const buffers = await Promise.all(
    Array.from({ length: totalPages }, (_, i) =>
      buildWkCanvas(
        allSorted,
        `Who Knows — ${canonicalTrack}`,
        `track by ${canonicalArtist}`,
        "plays",
  ...
  const urls = await Promise.all(
    buffers.map((buf, i) =>
      uploadToSupabase(
        buf,
        "wk-cache",
        `${interaction.guildId}_track_${encodeURIComponent(canonicalTrack)}_${i}.png`,
      ),
    ),
  );
  ...
  const container = new ContainerBuilder()
    .addTextDisplayComponents(
      new TextDisplayBuilder().setContent(
        `### ${E.tracks} Who Knows **${canonicalTrack}**?`,
      ),
    )
  ...
  if (totalPages > 1) {
    const row = new ActionRowBuilder().addComponents(
      new ButtonBuilder()
        .setCustomId(
          `wk_track_prev_0_${interaction.guildId}_${encodeURIComponent(canonicalTrack)}|||${encodeURIComponent(canonicalArtist)}`,
        )
        .setEmoji({
      ...
      new ButtonBuilder()
        .setCustomId(
          `wk_track_next_0_${interaction.guildId}_${encodeURIComponent(canonicalTrack)}|||${encodeURIComponent(canonicalArtist)}`,
        )
        .setEmoji({
