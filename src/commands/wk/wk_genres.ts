...
export async function executeWkGenres(interaction: any): Promise<void> {
  ...
  if (!genreInput) {
    ...
    if (!callerDb?.lastfmUsername) {
      const container = new ContainerBuilder().addTextDisplayComponents(
        new TextDisplayBuilder().setContent(
          `${E.reject} No genre specified and you haven't linked your Last.fm. Use ${cmdMention("link")} or specify a genre.`,
        ),
      );
    ...
    if (!np?.topGenre) {
      const container = new ContainerBuilder().addTextDisplayComponents(
        new TextDisplayBuilder().setContent(
          `${E.reject} Couldn't detect a genre from what you're listening to. Please specify one.`,
        ),
      );
      await interaction.editReply({
        components: [container],
    ...
    linkedMembers.map(async (m) => {
      try {
        ...
        for (let i = 0; i < artists.slice(0, 30).length; i++) {
          const tags: string[] = (artistInfos[i]?.artist?.tags?.tag ?? []).map(
  ...
  if (allSorted.length === 0) {
    const container = new ContainerBuilder().addTextDisplayComponents(
      new TextDisplayBuilder().setContent(
        `${E.reject} Nobody in this server listens to **${genreInput}**.`,
      ),
    );
    await interaction.editReply({
      components: [container],
  ...
  const urls = await Promise.all(
    buffers.map((buf, i) =>
      uploadToSupabase(
        buf,
        "wk-cache",
        `${interaction.guildId}_genre_${encodeURIComponent(genreInput!)}_${i}.png`,
      ),
    ),
  );

  // Save to generic cache
  const genericCacheKey = `wk_genres_${interaction.guildId}_${genreInput}`;
  await setCache(
    genericCacheKey,
    {
      imageUrls: urls,
  ...
  const container = new ContainerBuilder()
    .addTextDisplayComponents(
      new TextDisplayBuilder().setContent(
        `### ${E.chart} Who Listens to **${displayGenre}**?`,
      ),
    )
  ...
  if (totalPages > 1) {
    const row = new ActionRowBuilder().addComponents(
      new ButtonBuilder()
        .setCustomId(
          `wk_genre_prev_0_${interaction.guildId}_${encodeURIComponent(genreInput)}`,
        )
        .setEmoji({
      ...
      new ButtonBuilder()
        .setCustomId(
          `wk_genre_next_0_${interaction.guildId}_${encodeURIComponent(genreInput)}`,
        )
        .setEmoji({
