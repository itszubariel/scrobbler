...
// ── Card 1: Cover ─────────────────────────────────────────────────────────────
export async function buildCoverCard(
  username: string,
...
export async function buildArtistsCard(
  ...
  for (const p of podium) {
    ...
    ctx.fillText(
      truncate(ctx, artist.name, p.size + 20),
      p.x + p.size / 2,
      nameY,
    );
  ...
  if (top1) {
    ...
    ctx.textAlign = "right";
    ctx.fillText(
      `${top1.playcount.toLocaleString("en-US")} plays`,
      W - 40,
      420,
    );
    ctx.textAlign = "left";
...
export async function buildTracksCard(
  ...
  for (let i = 0; i < Math.min(5, tracks.length); i++) {
    ...
    ctx.textAlign = "right";
    ctx.fillText(
      `${track.playcount.toLocaleString("en-US")}`,
      770,
      rowY + ROW_H / 2 + 5,
    );
    ctx.textAlign = "left";
...
export async function buildStatsCard(
  ...
  const topGenreDisplay =
    stats.topGenre.charAt(0).toUpperCase() + stats.topGenre.slice(1);

  const boxes = [
    {
      x: 30,
      y: 55,
      value: stats.uniqueArtists.toLocaleString("en-US"),
      label: tracked("ARTISTS"),
    },
    {
      x: 290,
      y: 55,
      value: stats.uniqueTracks.toLocaleString("en-US"),
      label: tracked("TRACKS"),
    },
    {
      x: 550,
      y: 55,
      value: stats.uniqueAlbums.toLocaleString("en-US"),
      label: tracked("ALBUMS"),
    },
    {
      x: 30,
      y: 220,
      value: `${stats.discoveryScore}%`,
      label: tracked("UNDERGROUND"),
    },
    { x: 290, y: 220, value: topGenreDisplay, label: tracked("TOP GENRE") },
    {
      x: 550,
      y: 220,
      value: stats.dailyAvg.toLocaleString("en-US"),
      label: tracked("DAILY AVG"),
    },
  ];
  ...
  for (const box of boxes) {
    ...
    ctx.textAlign = "center";
    ctx.fillText(
      truncate(ctx, box.value, BW - 16),
      box.x + BW / 2,
      box.y + BH / 2 + (isLong ? 8 : 14),
    );
