...
async function buildGenresLeaderboardImage(
  ...
  members.forEach((member, i) => {
    ...
    ctx.textAlign = 'right';
    ctx.fillText(`${member.count.toLocaleString('en-US')} genres`, WIDTH - 30, TEXT_Y);
    ctx.textAlign = 'left';
  ...
  ctx.fillText(`Total unique genres: ${totalGenres}`, WIDTH / 2, footerY + 36);
  ctx.textAlign = 'left';
...
export async function executeStatsGenres(interaction: any): Promise<void> {
  ...
  const imageBuffer = await buildGenresLeaderboardImage(members, interaction.guild.name);
  ...
  const container = new ContainerBuilder()
    ...
    .addTextDisplayComponents(
      new TextDisplayBuilder().setContent(`-# ${members.length} members • Unique genres from top 50 artists`)
    );
