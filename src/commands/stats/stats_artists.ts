...
export async function executeStatsArtists(interaction: any): Promise<void> {
  ...
  const footerParts = [`${members.length} members • Unique artists overall`];
  if (callerRank > 10 && callerEntry) {
    footerParts.push(`You are ranked **#${callerRank}** with **${callerEntry.displayCount}** artists`);
  }

  const imageBuffer = await buildLeaderboardCanvas(members.slice(0, 10), interaction.guild.name, "artists", `Total unique artists: ${totalArtists}`);
