...
export async function executeStatsArtists(interaction: any): Promise<void> {
  ...
  const artistResults = await Promise.all(
    linkedMembers.map(m =>
      fetch(`https://ws.audioscrobbler.com/2.0/?method=user.gettopartists&user=${encodeURIComponent(m.user.lastfmUsername!)}&limit=1000&period=overall&api_key=${apiKey}&format=json`)
        .then(r => r.json()).catch(() => null)
    )
  ) as any[];

  const allMembers = linkedMembers.map((m, i) => {
    const raw = (artistResults[i]?.topartists?.artist ?? []).length as number;
    return { username: m.user.lastfmUsername!, count: raw, displayCount: raw >= 1000 ? '1,000+' : raw.toLocaleString('en-US') };
  }).sort((a, b) => b.count - a.count);

  const hasCapHit = allMembers.some(m => m.count >= 1000);
  const totalArtists = (hasCapHit ? allMembers.reduce((s, m) => s + m.count, 0).toLocaleString('en-US') + '+' : allMembers.reduce((s, m) => s + m.count, 0).toLocaleString('en-US'));
  ...
  const buffers = await Promise.all(
    Array.from({ length: totalPages }, (_, i) => buildLeaderboardCanvas(allMembers, interaction.guild.name, 'artists', `Total unique artists: ${totalArtists}`, i))
  );
  const urls = await Promise.all(
    buffers.map((buf, i) => uploadToSupabase(buf, 'stats-cache', `artists_${interaction.guildId}_${i}.png`))
  );

  await (prisma as any).statsArtistsCache.upsert({
    where: { guildId: interaction.guildId },
  ...
  const footerParts = [`${memberCount} members • Unique artists overall`];
  if (callerRank > PAGE_SIZE && callerEntry) footerParts.push(`You are ranked **#${callerRank}** with **${callerEntry.displayCount}** artists`);

  const container = new ContainerBuilder()
    .addTextDisplayComponents(new TextDisplayBuilder().setContent(`### ${E.artists} Server Artist Leaderboard — All time`))
  ...
  if (totalPages > 1) {
    ...
    const row = new ActionRowBuilder().addComponents(
      new ButtonBuilder().setCustomId(`stats_artists_prev_0_${authorId}`).setEmoji({ id: E.prev.match(/:(\d+)>/)?.[1] ?? '0', name: 'scrobbler_prev' }).setStyle(ButtonStyle.Secondary).setDisabled(true),
      new ButtonBuilder().setCustomId(`stats_artists_next_0_${authorId}`).setEmoji({ id: E.next.match(/:(\d+)>/)?.[1] ?? '0', name: 'scrobbler_next' }).setStyle(ButtonStyle.Secondary).setDisabled(false),
    );
