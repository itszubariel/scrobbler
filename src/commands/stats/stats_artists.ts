...
export async function executeStatsArtists(interaction: any): Promise<void> {
  ...
  // Check cache first
  const cacheKey = `stats_artists_${interaction.guildId}`;
  ...
  if (cached && cached.imageUrls && cached.imageUrls.length > 0) {
    ...
    const footerParts = [
      `${cached.memberCount} members • Unique artists overall`,
    ];

    const container = new ContainerBuilder()
      .addTextDisplayComponents(
        new TextDisplayBuilder().setContent(
          `### ${E.artists} Server Artist Leaderboard — All time`,
        ),
      )
    ...
    if (cached.pageCount > 1) {
      ...
      const row = new ActionRowBuilder().addComponents(
        new ButtonBuilder()
          .setCustomId(`stats_artists_prev_0_${authorId}`)
          .setEmoji({
          ...
          .setDisabled(true),
        new ButtonBuilder()
          .setCustomId(`stats_artists_next_0_${authorId}`)
          .setEmoji({
  ...
  const artistResults = (await Promise.all(
    linkedMembers.map((m) =>
      fetch(
        `https://ws.audioscrobbler.com/2.0/?method=user.gettopartists&user=${encodeURIComponent(m.user.lastfmUsername!)}&limit=1000&period=overall&api_key=${apiKey}&format=json`,
      )
        .then((r) => r.json())
        .catch(() => null),
    ),
  ...
  const buffers = await Promise.all(
    Array.from({ length: totalPages }, (_, i) =>
      buildLeaderboardCanvas(
        ...
        "artists",
        `Total unique artists: ${totalArtists}`,
        i,
      ),
    ),
  );
  const urls = await Promise.all(
    buffers.map((buf, i) =>
      uploadToSupabase(
        buf,
        "stats-cache",
        `artists_${interaction.guildId}_${i}.png`,
      ),
    ),
  );
  ...
  if (callerRank > PAGE_SIZE && callerEntry)
    footerParts.push(
      `You are ranked **#${callerRank}** with **${callerEntry.displayCount}** artists`,
    );
