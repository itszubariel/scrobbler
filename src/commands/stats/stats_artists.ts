...
async function buildArtistsImage(
  ...
  const totalArtists = members.reduce((sum, m) => sum + m.count, 0).toLocaleString('en-US');
  ...
  members.forEach((member, i) => {
    ...
    ctx.textAlign = 'right';
    ctx.fillText(`${member.count.toLocaleString('en-US')} artists`, WIDTH - 30, TEXT_Y);
    ctx.textAlign = 'left';
  ...
  ctx.fillText(`Total unique artists: ${totalArtists}`, WIDTH / 2, footerY + 36);
  ctx.textAlign = 'left';
...
export async function executeStatsArtists(interaction: any): Promise<void> {
  ...
  const members = linkedMembers
    .map((m, i) => ({
      ...
      count: (artistResults[i]?.topartists?.artist ?? []).length as number,
    }))
    .sort((a, b) => b.count - a.count);

  const imageBuffer = await buildArtistsImage(members, interaction.guild.name);
  const attachment = new AttachmentBuilder(imageBuffer, { name: 'stats_artists.png' });

  const container = new ContainerBuilder()
    .addTextDisplayComponents(
      new TextDisplayBuilder().setContent(`### ${E.artists} Server Artist Leaderboard — All time`)
    )
    ...
    .addMediaGalleryComponents(
      new MediaGalleryBuilder().addItems(
        new MediaGalleryItemBuilder().setURL('attachment://stats_artists.png')
      )
    )
    .addSeparatorComponents(
      new SeparatorBuilder().setDivider(true).setSpacing(SeparatorSpacingSize.Small)
    ...
    .addTextDisplayComponents(
      new TextDisplayBuilder().setContent(`-# ${members.length} members • Unique artists overall`)
    );
