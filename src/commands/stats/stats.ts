...
export async function fetchStatsData(
  guildId: string,
  apiKey: string,
): Promise<{ members: StatsEntry[] } | null> {
  ...
  const infoResults = (await Promise.all(
    linkedMembers.map((m) =>
      fetch(
        `https://ws.audioscrobbler.com/2.0/?method=user.getinfo&user=${encodeURIComponent(m.user.lastfmUsername!)}&api_key=${apiKey}&format=json`,
      )
        .then((r) => r.json())
        .catch(() => null),
    ),
...
export async function buildStatsImage(
  ...
  const totalScrobbles = members
    .reduce((sum, m) => sum + m.scrobbles, 0)
  ...
  const HEIGHT = Math.max(
    400,
    HEADER_H + pageMembers.length * ROW_H + FOOTER_H,
  );
  ...
  pageMembers.forEach((member, i) => {
    ...
    const rankColor =
      rank === 1 ? "#ffffff" : rank <= 3 ? "#aaaaaa" : "#666666";
    ...
    ctx.textAlign = "right";
    ctx.fillText(
      `${member.scrobbles.toLocaleString("en-US")} scrobbles`,
      WIDTH - 30,
      TEXT_Y,
    );
    ctx.textAlign = "left";
    ...
    if (fillRatio > 0) {
      ctx.fillStyle =
        rank === 1 ? "#ffffff" : rank <= 3 ? "#aaaaaa" : "#555555";
      ctx.beginPath();
  ...
  ctx.fillText(
    `Total server scrobbles: ${totalScrobbles}`,
    WIDTH / 2,
    footerY + 36,
  );
  ctx.textAlign = "left";
...
export const statsCommand: Command = {
  data: new SlashCommandBuilder()
    ...
    .addSubcommand((sub) =>
      sub
        .setName("scrobbles")
        .setDescription("Who has scrobbled the most in this server"),
    )
    .addSubcommand((sub) =>
      sub
        .setName("artists")
        .setDescription("Who has listened to the most unique artists"),
    )
    .addSubcommand((sub) =>
      sub
        .setName("albums")
        .setDescription("Who has listened to the most unique albums"),
    )
    .addSubcommand((sub) =>
