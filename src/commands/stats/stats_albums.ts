...
export async function executeStatsAlbums(interaction: any): Promise<void> {
  ...
  const totalAlbums = hasCapHit ? allMembers.reduce((s, m) => s + m.count, 0).toLocaleString('en-US') + '+' : allMembers.reduce((s, m) => s + m.count, 0).toLocaleString('en-US');
  ...
  const buffers = await Promise.all(
    Array.from({ length: totalPages }, (_, i) => buildLeaderboardCanvas(allMembers, interaction.guild.name, 'albums', `Total unique albums: ${totalAlbums}`, i))
  );
  const urls = await Promise.all(
    buffers.map((buf, i) => uploadToSupabase(buf, 'stats-cache', `albums_${interaction.guildId}_${i}.png`))
  );
  ...
  if (callerRank > PAGE_SIZE && callerEntry) footerParts.push(`You are ranked **#${callerRank}** with **${callerEntry.displayCount}** albums`);

  const container = new ContainerBuilder()
    .addTextDisplayComponents(new TextDisplayBuilder().setContent(`### ${E.albums} Server Album Leaderboard — All time`))
  ...
  if (totalPages > 1) {
    ...
    const row = new ActionRowBuilder().addComponents(
      new ButtonBuilder().setCustomId(`stats_albums_prev_0_${authorId}`).setEmoji({ id: E.prev.match(/:(\d+)>/)?.[1] ?? '0', name: 'scrobbler_prev' }).setStyle(ButtonStyle.Secondary).setDisabled(true),
      new ButtonBuilder().setCustomId(`stats_albums_next_0_${authorId}`).setEmoji({ id: E.next.match(/:(\d+)>/)?.[1] ?? '0', name: 'scrobbler_next' }).setStyle(ButtonStyle.Secondary).setDisabled(false),
    );
