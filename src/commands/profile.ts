...
export const profileCommand: Command = {
  ...
  async execute(interaction) {
    ...
    const [infoData, topArtistsData, topTracksData, topTagsData] =
      (await Promise.all([
        fetch(
          `https://ws.audioscrobbler.com/2.0/?method=user.getinfo&user=${encodeURIComponent(lfmUsername)}&api_key=${apiKey}&format=json`,
        ).then((r) => r.json()),
        fetch(
          `https://ws.audioscrobbler.com/2.0/?method=user.gettopartists&user=${encodeURIComponent(lfmUsername)}&period=overall&limit=5&api_key=${apiKey}&format=json`,
        ).then((r) => r.json()),
        fetch(
          `https://ws.audioscrobbler.com/2.0/?method=user.gettoptracks&user=${encodeURIComponent(lfmUsername)}&period=overall&limit=1&api_key=${apiKey}&format=json`,
        ).then((r) => r.json()),
        fetch(
    ...
    const registeredYear = new Date(
      parseInt(user.registered?.unixtime ?? user.registered?.["#text"] ?? "0") *
        1000,
    ).getFullYear();
    const country =
      user.country && user.country !== "None" ? user.country : null;
    ...
    const avatar =
      user.image?.find((img: any) => img.size === "extralarge")?.["#text"] ??
    ...
    const wrapText = (text: string, maxLen = 45): string => {
      ...
      for (const word of words) {
        if (
          (current + (current ? " " : "") + word).length > maxLen &&
          current
        ) {
          lines.push(current);
          current = word;
    ...
    const use2x2 =
      scrobblesRaw > 99_999 ||
      artistCountRaw > 10_000 ||
      trackCountRaw > 10_000 ||
      albumCountRaw > 10_000;
