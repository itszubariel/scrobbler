...
export async function fetchWrappedData(lfmUsername: string, period: string, apiKey: string): Promise<WrappedPayload | null> {
  ...
  const [userInfoRes, topArtistsRes, topTracksRes] = await Promise.all([
    ...
    fetch(`https://ws.audioscrobbler.com/2.0/?method=user.gettopartists&user=${encodeURIComponent(lfmUsername)}&period=${period}&limit=10&api_key=${apiKey}&format=json`)
      .then(r => r.json()).catch(() => null),
    fetch(`https://ws.audioscrobbler.com/2.0/?method=user.gettoptracks&user=${encodeURIComponent(lfmUsername)}&period=${period}&limit=10&api_key=${apiKey}&format=json`)
      .then(r => r.json()).catch(() => null),
  ]) as any[];
  ...
  const artistImages: (string | null)[] = topArtists.slice(0, 5).map((a: any, i: number) => {
  ...
  const trackImages: (string | null)[] = topTracks.slice(0, 5).map((_: any, i: number) => {
  ...
  const sortedGenres = [...tagWeights.entries()].sort((a, b) => b[1] - a[1]).slice(0, 5);
  ...
  return {
    ...
    trackCount,
    albumCount,
    topArtists: topArtists.map((a: any) => ({ name: a.name, playcount: parseInt(a.playcount ?? '0') })),
    artistImages,
    topTracks: topTracks.map((t: any) => ({ name: t.name, artist: t.artist?.name ?? '', playcount: parseInt(t.playcount ?? '0') })),
    trackImages,
    topGenres,
    discoveryScore,
    daysInPeriod,
...
export async function buildWrappedCard(payload: WrappedPayload, page: number): Promise<Buffer> {
  const dailyAvg = payload.daysInPeriod > 0 ? Math.round(payload.scrobbles / payload.daysInPeriod) : payload.scrobbles;
  const topGenre = payload.topGenres[0]?.name ?? '—';

  switch (page) {
    case 1: return buildCoverCard(payload.username, payload.scrobbles);
    case 2: return buildArtistsCard(payload.username, payload.topArtists, payload.artistImages);
    case 3: return buildTracksCard(payload.username, payload.topTracks, payload.trackImages);
    case 4: return buildTasteCard(payload.username, payload.topGenres, payload.discoveryScore);
    case 5: return buildStatsCard(payload.username, {
    ...
    default: return buildCoverCard(payload.username, payload.scrobbles);
  }
}
...
export function buildWrappedContainer(
  ...
  const container = new ContainerBuilder()
    ...
    .addTextDisplayComponents(
      new TextDisplayBuilder().setContent(`-# Page ${page} of ${TOTAL} • ${periodLabel}`)
    )
    .addSeparatorComponents(
...
async function uploadToSupabase(buffer: Buffer, discordId: string, page: number): Promise<string> {
  ...
  if (!res.ok) {
    ...
    console.error(`Supabase upload failed for page ${page}: ${res.status} ${text}`);
  }
...
export const wrappedCommand: Command = {
  data: new SlashCommandBuilder()
    ...
    .addUserOption(option =>
      option.setName("user").setDescription("Check another user's Wrapped (optional)").setRequired(false)
    ),
  ...
  async execute(interaction) {
    ...
    const buffers = await Promise.all([1, 2, 3, 4, 5].map(p => buildWrappedCard(payload, p)));

    // Upload all 5 to Supabase Storage in parallel
    const urls = await Promise.all(buffers.map((buf, i) => uploadToSupabase(buf, targetDiscordUser.id, i + 1)));
    ...
    await db.wrappedCache.upsert({
      ...
      update: { urls, period, expiresAt: new Date(Date.now() + 60 * 60 * 1000) },
      create: { discordId: targetDiscordUser.id, urls, period, expiresAt: new Date(Date.now() + 60 * 60 * 1000) },
    });

    const page = 1;
    const container = buildWrappedContainer(payload.username, payload.periodLabel, page, targetDiscordUser.id, urls[0]!);
