...
function getPersonalityType(dims: Record<string, number>): PersonalityType {
  ...
  if (within10) {
    ...
    if (combo.has("diversity") && combo.has("mainstream"))
      return {
        name: "🎪 The Curator",
        description: "You know every genre but always know what's trending.",
      };
  ...
  if (top === "intensity")
    return {
      name: "🔥 The Obsessive",
      description: "Music isn't background noise for you. It's everything.",
    };
...
export const personalityCommand: Command = {
  data: new SlashCommandBuilder()
    ...
    .addUserOption((option) =>
      option
        .setName("user")
        .setDescription("Check another user's personality (optional)")
        .setRequired(false),
    ),
  ...
  async execute(interaction) {
    ...
    const [overallRes, weekRes, userInfoRes] = (await Promise.all([
      fetch(
        `https://ws.audioscrobbler.com/2.0/?method=user.gettopartists&user=${encodeURIComponent(lfmUsername)}&period=overall&limit=100&api_key=${apiKey}&format=json`,
      )
        .then((r) => r.json())
        .catch(() => null),
      fetch(
        `https://ws.audioscrobbler.com/2.0/?method=user.gettopartists&user=${encodeURIComponent(lfmUsername)}&period=7day&limit=50&api_key=${apiKey}&format=json`,
      )
        .then((r) => r.json())
        .catch(() => null),
      fetch(
    ...
    const overallNames = new Set(
      overallArtists.map((a) => a.name.toLowerCase()),
    );
    const weekInOverall = weekArtists.filter((a) =>
      overallNames.has(a.name.toLowerCase()),
    ).length;
    const loyalty =
      weekArtists.length > 0
        ? Math.round((weekInOverall / weekArtists.length) * 100)
        : 50;
    ...
    const topGenrePercentage =
      totalTagWeight > 0 && sortedTags.length > 0
    ...
    for (let i = 0; i < overallArtists.length; i++) {
      ...
      const playcount = parseInt(artist.playcount ?? "0") || 1;
      totalWeightedScore +=
        Math.min(listeners / 5_000_000, 1) * 100 * playcount;
    ...
    const registeredUnix =
      parseInt(
        userInfoRes?.user?.registered?.unixtime ??
          userInfoRes?.user?.registered?.["#text"] ??
          "0",
      ) || 0;
    const monthsSinceReg =
      registeredUnix > 0
        ? Math.max(
            1,
            (Date.now() / 1000 - registeredUnix) / (30.44 * 24 * 3600),
          )
        : 12;
    const intensity = Math.min(
      Math.round((totalScrobbles / monthsSinceReg / 300) * 100),
      100,
    );
